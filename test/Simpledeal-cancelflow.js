...
contract('HashtagDeal', function(accounts) {
  ...
  describe('SimpleDeal flow', function() {
    ...
    it("should read the dealStatus", function(done) {
      dealContract.dealStatus.call().then(function(status) {
        console.log('dealstatus=',status);
        done();
      });
    });
    ...
    it("should cancel the SimpleDeal", function(done) {
      dealContract.cancel({
        from: accounts[1]
      }).then(function(instance) {
        done();
    ...
    it("should see tokens refunded to accounts[1]", function(done) {
      swtToken.balanceOf(accounts[1]).then(function(balance) {
        assert.equal(balance, 101, "accounts[1] balance not correct after cancel");
        console.log('Balance of account=', balance.toNumber());
        done();
      });
    });
