...
contract('HashtagSimpleDeal', function(accounts) {
  ...
  describe('Dispute by seeker', function() {
    ...
    it("should payout the deal", function(done) {
      ...
      it("should check if the deal is in status 1 [done]", function(done) {
        var dealhash = web3.sha3(cleardealid);

        hashtagContract.readDeal.call(dealhash).then(function(res) {
          //console.log(res[0].toNumber(), res[1].toNumber(), res[2].toNumber(), res[3]);
          assert.equal(res[0].toNumber(), 1, "deal status not correct after resolve");
          done();
        });
      });
    ...
    it("should see token balance increased on providers's account", function(done) {
      swtToken.balanceOf(provider).then(function(balance) {
        //console.log('Provider account: ', balance.toNumber());
        assert.equal(balance.toNumber(), 100900000000000000000, "deal balance not correct after funding");
    ...
    it("should see 5 reptoken balance on ProviderRepToken", function(done) {
      hashtagProviderRepToken.balanceOf(provider).then(function(balance) {
        //console.log('Proverderrep balance: ', balance.toNumber())
        assert.equal(balance.toNumber(), 5, "deal balance not correct after funding");
        done();
      });
    });

    it("should see 5 reptoken balance on SeekerRepToken", function(done) {
  ...
  describe('Stats', function() {
    it("should show stats of conflict > payout", function(done) {
      var cumulatedGas = 0;
